...
class ChatDetailPage extends StatelessWidget {
            ...
            builder: (context, itemSnapshot) {
              ...
              return SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    _buildActionSection(context, isOwner, itemId, ownerId, visitorId, itemData),
  ...
  Widget _buildActionSection(BuildContext context, bool isOwner, String itemId, String ownerId, String visitorId, Map<String, dynamic> itemData) {
      ...
      return Container(
        ...
        child: Column(
          children: const [
            ...
            Text(
              'ภารกิจเสร็จสิ้น',
              style: TextStyle(fontFamily: 'Line Seed Sans TH', fontSize: 18, fontWeight: FontWeight.bold, color: Colors.green),
            ),
            Text('ปิดงานเรียบร้อยแล้ว', style: TextStyle(fontFamily: 'Line Seed Sans TH', color: Colors.green)),
          ],
        ),
      );
    }
        ...
        if (isOngoing)
           if (isOwner)
             ...
             Container(
               ...
               child: const Center(child: Text("รอเจ้าของโพสต์ตรวจสอบและปิดงาน", style: TextStyle(fontFamily: 'Line Seed Sans TH', color: Colors.orange, fontWeight: FontWeight.bold))),
             )
      ],
    );
  }

  Widget _buildAppointmentButton(BuildContext context, String itemId, Map<String, dynamic> itemData) {
    bool hasAppointment = itemData['appointment_time'] != null;
    String btnLabel = hasAppointment ? 'แก้ไขเวลานัดหมาย' : 'นัดหมายสถานที่และเวลา';
    IconData btnIcon = hasAppointment ? Icons.edit_calendar : Icons.calendar_today;
  ...
  Widget _buildCompletionButton(BuildContext context, String itemId, Map<String, dynamic> itemData, String visitorId) {
    return SizedBox(
      width: double.infinity,
      height: 50,
      child: ElevatedButton.icon(
        onPressed: () => _confirmCompletion(context, itemId, itemData, visitorId),
  ...
  Future<void> _showAppointmentDialog(BuildContext context, String itemId, Map<String, dynamic> itemData) async {
    DateTime selectedDate = DateTime.now();
    TimeOfDay selectedTime = TimeOfDay.now();
    TextEditingController locController = TextEditingController(text: itemData['appointment_location'] ?? '');
      ...
      builder: (ctx) {
        return StatefulBuilder(
          builder: (context, setState) {
            return AlertDialog(
              title: const Text('นัดหมาย', style: TextStyle(fontFamily: 'Line Seed Sans TH', fontWeight: FontWeight.bold)),
              content: SizedBox(
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      GestureDetector(
                        onTap: () async {
                          LatLng? result = await _selectLocationOnMap(context, selectedLatLng);
                          if (result != null) {
                            setState(() {
                          ...
                          child: selectedLatLng == null
                              ? Column(
                                  ...
                                  children: const [
                                    ...
                                    Text("แตะเพื่อเลือกตำแหน่งบนแผนที่", style: TextStyle(color: Colors.grey)),
                                  ],
                                )
                              : ClipRRect(
                                  borderRadius: BorderRadius.circular(8),
              ...
              actions: [
                ...
                ElevatedButton(
                  style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF006C68), foregroundColor: Colors.white),
                  onPressed: () async {
                    Navigator.pop(ctx);
                    await _saveAppointment(context, itemId, itemData['status'] ?? 'lost', locController.text, selectedDate, selectedTime, selectedLatLng);
                  },
  ...
  Future<LatLng?> _selectLocationOnMap(BuildContext context, LatLng? currentSelection) async {
  ...
  Future<void> _saveAppointment(BuildContext context, String itemId, String currentStatus, String location, DateTime date, TimeOfDay time, LatLng? latLng) async {
    try {
      final currentUser = FirebaseAuth.instance.currentUser;
      ...
      if (latLng != null) {
        updateData['appointment_geo'] = GeoPoint(latLng.latitude, latLng.longitude);
      }
      ...
      String formattedDate = "${dt.day}/${dt.month}/${dt.year} เวลา ${time.format(context)}";
      String msg = "📅 มีการนัดหมาย/เปลี่ยนแปลงเวลา\nสถานที่: $location\nเวลา: $formattedDate";
      ...
      if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('บันทึกการนัดหมายเรียบร้อย')));
  ...
  Future<void> _confirmCompletion(BuildContext context, String itemId, Map<String, dynamic> itemData, String visitorId) async {
    // Calculate Points
    String status = itemData['status'] ?? 'lost';
    ...
    try {
      ...
      if (targetUserId.isNotEmpty) {
         ...
         }).catchError((e) => debugPrint("Error giving points: $e")); // Handle if user doc doesn't exist
      }
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('ปิดงานสำเร็จ! มอบคะแนนเรียบร้อย')));
      }
    } catch (e) {
